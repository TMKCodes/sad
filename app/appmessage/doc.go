/*
Package appmessage implements the sad appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported sad messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# sad Message Overview

The sad protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which sad network it is a part of, its type, how big it is, and a checksum
...
of the currently supported sad messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using sad encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the sad messages are intended
...
and write sad messages. The following sections provide a quick overview of
...
The sad network is a magic number which is used to identify the start of a
message and which sad network the message applies to. This package provides
...
As discussed in the sad message overview section, this package reads
and writes sad messages using a generic interface named Message. In
...
In order to unmarshall sad messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a sad peer. Example syntax is:

	// Reads and validates the next sad message from conn using the
	// protocol version pver and the sad network sadNet. The returns
	// are a appmessage.Message, a []byte which contains the unmarshalled
	// raw payload, and a possible error.
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, sadNet)
	if err != nil {
...
In order to marshall sad messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a sad peer. Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the sad network sadNet. The return is a possible
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, sadNet)
	if err != nil {
		// Log and handle the error
	}
