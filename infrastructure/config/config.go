...
const (
	defaultConfigFilename      = "sad.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "sad.log"
	defaultErrLogFilename      = "sad_err.log"
	...
	sampleConfigFilename    = "sample-sad.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for sad.
	DefaultAppDir = util.AppDir("sad", false)
...
// Flags defines the configuration options for sad.
//
...
// Config defines the configuration options for sad.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default sad configuration
...
//
// The above results in sad functioning properly without any config settings
...
// createDefaultConfig copies the file sample-sad.conf to the given destination path,
