...
Clients are expected to build RequestMessages and wrap them in SadMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a SadMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a SadMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network Sad is currently running against.
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether Sad thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when Sad is not synced. That is because when Sad isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known Sad addresses in the
...
currently connected to this Sad.
...
| isOutbound | [bool](#bool) |  | Whether this Sad initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this Sad |
...
AddPeerRequestMessage adds a peer to Sad&#39;s outgoing connection list.
This will, in most cases, result in Sad connecting to said peer.
...
parent chain from some startHash to this Sad&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this Sad.
...
ShutDownRequestMessage shuts down this Sad.
...
This call is only available when this Sad was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given Sad addresses
