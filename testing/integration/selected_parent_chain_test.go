...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of sad instances
	sad1, sad2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := sad1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In sad2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, sad2)
	}

	// Connect the two sads. This should trigger sync
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := sad1.rpcClient.GetVirtualSelectedParentChainFromBlock(
