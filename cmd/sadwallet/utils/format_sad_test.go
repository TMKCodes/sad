...
// Takes in a string representation of the Sad value to convert to Seep
func TestSadToSeep(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := SadToSeep(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := SadToSeep(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateSADAmountFormat(testCase)
