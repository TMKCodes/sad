...
// FormatSAD takes the amount of seeps as uint64, and returns amount of SAD with 8  decimal places
func FormatSad(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SeepPerSad)
	}
	return res
}

// SadToSeep takes in a string representation of the SAD value to convert to Seep
func SadToSeep(amount string) (uint64, error) {
	err := validateSADAmountFormat(amount)
	...
	if constants.SeepPerSad%10 != 0 {
		return 0, errors.Errorf("Unable to convert to seep when SeepPerSad is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SeepPerSad))
	decimalStr := ""
...
func validateSADAmountFormat(amount string) error {
