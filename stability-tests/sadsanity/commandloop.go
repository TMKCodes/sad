...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("sadsanity-sad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("sad", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "SAD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "SAD-STDERR")
