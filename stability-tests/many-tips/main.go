...
import (
	...
	"github.com/sadnetwork/sad/domain/consensus/utils/mining"
	...
	"github.com/sadnetwork/sad/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("sad-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("sad datadir: %s", dataDir)

	SadCmd, err := common.StartCmd("SAD",
		"sad",
	...
	spawn("startNode-SadCmd.Wait", func() {
		err := SadCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("SadCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes Sad closes ungracefully
				log.Errorf("SadCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
